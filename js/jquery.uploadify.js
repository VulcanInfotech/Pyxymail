		...
		init : function (options, swfUploadOptions) {
			...
			return this.each(function () {
				...
				var settings = $.extend({
						...
						buttonText : 'SELECT IMAGES', // The text to use for the browse button
						checkExisting : false, // The path to a server-side script that checks for existing files on the server
						debug : false, // Turn on swfUpload debugging mode
						fileObjName : 'Filedata', // The name of the file object to use in your server-side script
						fileSizeLimit : 1024, // The maximum size of an uploadable file in KB (Accepts units B KB MB GB if string, 0 for no limit)
						fileTypeDesc : 'jpg,jpeg,gif,png', // The description for file types in the browse dialog
						//fileTypeExts : 'image/*', // Allowed extensions in the browse dialog (server-side validation should also be used)
						...
						queueSizeLimit : 10, // The maximum number of files that can be in the queue at one time
						removeCompleted : true, // Remove queue items from the queue when they are done uploading
						removeTimeout : 5, // The delay in seconds before removing a queue item if removeCompleted is set to true
				...
				if (flashInstalled) {
					...
					var $button = $('<div />', {
							...
							'class' : 'file-selector ' + settings.buttonClass
						});
					...
					$button.html('<span class="file-selector-text">' + settings.buttonText + '</span>')
					.css({
						'height' : settings.height + 'px',
					...
					swfuploadify.queueData = {
						...
						filesCancelled : 1, // The number of files that were cancelled instead of replaced
			...
			this.each(function () {
				...
				if (args.length === 1) {
					...
					case 'buttonCursor':
						...
						break;
					case 'buttonText':
						$('#' + settings.id + '-button').find('.file-selector-text').html(value);
						break;
					case 'width':
